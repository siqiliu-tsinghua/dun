...
`scripts/release-build.sh` runs here. It is POSIX `sh` precisely because the
FreeBSD base system has no `bash`; if you add a bashism to it, this guest is
where that breaks. The last recorded figure is 698,344 bytes, the smallest of
the four platforms.

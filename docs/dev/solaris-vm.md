...
**The Solaris build carries one extra link flag.** `scripts/release-build.sh`
adds `-C link-arg=-znoldynsym` on SunOS, which drops the `.SUNW_ldynsym`
section, its two sort tables, and the `.dynstr` entries they need — about
343 KB of symbol-table metadata that the native link editor keeps so `pstack`
and `dtrace` can name local frames. Without it the binary is 1,087,760 bytes;
with it, 744,880, in line with the other three platforms. The cost is that
stack traces from a release binary name only exported symbols; panic messages
still carry `file:line`, and this build never linked backtrace symbolization
anyway. Set `DUN_SOLARIS_KEEP_LDYNSYM=1` to keep the section — necessary if you
link with GNU `ld`, which does not know the option.

Two neighbouring flags do **not** help and should not be tried: `strip` and
`strip -x` move zero bytes (the profile already sets `strip = "symbols"`, and
what remains is allocable), and `-z strip-class=nonalloc` makes the binary
377 KB *larger*. Measurements and mechanism:

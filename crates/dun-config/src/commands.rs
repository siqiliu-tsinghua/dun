...
/// updated too. `EditorCommand::PluginAction` is deliberately absent: it is
...
pub fn command_id(command: &EditorCommand) -> &'static str {
    match command {
        ...
        EditorCommand::Edit(EditCommand::PasteExternal) => "edit.paste_external",
        ...
        // Plugin actions are never user-bindable, so every instance collapses
        // to one generic id and has no `command_from_id` round-trip.
        EditorCommand::PluginAction { .. } => "plugin.action",
    }
}
...
pub fn command_from_id(input: &str) -> Result<EditorCommand, CommandParseError> {
    match normalize_command_id(input).as_str() {
        ...
        "edit.paste_external" => Ok(EditorCommand::Edit(EditCommand::PasteExternal)),
